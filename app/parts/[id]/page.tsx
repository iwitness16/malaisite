...
import { getPartById } from '@/lib/firebase';
import { Part } from '@/lib/types';
import { useCart } from '@/lib/context';
import { Heart, Copy, Check, Share2, MessageCircle, Mail } from 'lucide-react';
import Link from 'next/link';

// ─── Share Panel ──────────────────────────────────────────────────────────────
function SharePanel({ part }: { part: Part }) {
  ...
  const url =
    typeof window !== 'undefined'
      ...
      : `${process.env.NEXT_PUBLIC_SITE_URL ?? 'https://elitepartz.com'}/parts/${part.id}`;
    ...
    <div className="relative">
      ...
      {open && (
        <div className="absolute left-0 bottom-full mb-2 w-80 bg-white border border-gray-200 shadow-2xl rounded-lg p-4 z-50">
          ...
          {/* Tip for Instagram / TikTok */}
          <div className="mb-3 text-xs text-gray-500 bg-gray-50 border border-gray-100 rounded p-2">
            <strong>Instagram / TikTok:</strong> Copy the link above, then paste it in your bio, story, or caption so followers can tap directly to this product.
          </div>

          {/* Social buttons */}
          <div className="grid grid-cols-2 gap-2">
            {shareTargets.map(({ label, href, bg, icon }) => (
              <a
                key={label}
                href={href}
                target="_blank"
...
export default function ProductDetailPage() {
  ...
  const [showTab,       setShowTab]       = useState<'details' | 'description' | 'questions'>('details');

  // Fetch by direct document ID — fastest possible lookup
  useEffect(() => {
    if (!id) return;
    setLoading(true);
    setActiveImage(0);
    ...
    getPartById(id)
      .then((found) => setPart(found))
      .catch((err)  => console.error('Error fetching part:', err))
  ...
  if (!part) {
    return (
      <div className="px-4 sm:px-6 lg:px-8 py-20 text-center">
        ...
        <Link href="/parts" className="text-red-600 hover:text-red-700 font-semibold">
          ← Back to all parts
        </Link>
      </div>
    );
    ...
    <div className="min-h-screen bg-white">
      <div className="px-4 sm:px-6 lg:px-8 py-8 max-w-7xl mx-auto">
        ...
        <nav className="flex items-center gap-2 text-sm text-gray-500 mb-8" aria-label="Breadcrumb">
          ...
          <span>/</span>
          <Link href="/parts" className="hover:text-gray-900 transition">Parts</Link>
          <span>/</span>
          <span className="text-gray-900 font-medium truncate max-w-[200px]">{part.name}</span>
        </nav>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
            ...
            {images.length > 1 && (
              <div className="flex gap-2 flex-wrap">
                {images.map((src, idx) => (
                  ...
                  >
                    <img src={src} alt={`${part.name} thumbnail ${idx + 1}`} className="w-full h-full object-cover" />
                    {idx === 0 && (
          ...
          <div>
            {/* Stock badge */}
            <span className={`inline-flex items-center gap-1.5 mb-4 px-3 py-1.5 text-xs font-bold tracking-widest rounded border ${
              part.inStock
            ...
            {/* Price */}
            <p className="text-4xl font-black text-red-600 mb-6">${part.price.toFixed(2)}</p>

            {/* Application tags */}
            {part.application.length > 0 && (
              <div className="mb-5">
                <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Application</p>
                <div className="flex flex-wrap gap-2">
                  {part.application.map((app) => (
                    <span key={app} className="px-3 py-1 bg-gray-100 text-gray-700 text-sm rounded-full">
                      {app}
                    </span>
                  ))}
                </div>
            ...
            <div className="mb-6 space-y-1.5 text-sm">
              ...
              <p>
                ...
                {part.make === 'All Models' ? 'Compatible with all models' : part.make}
              </p>
            </div>
        ...
        <div className="mt-14 border-t pt-8">
          <div className="flex gap-0 border-b mb-6">
            {(['details', 'description', 'questions'] as const).map((tab) => (
              <button
                key={tab}
          ...
          {showTab === 'details' && (
            <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-3 text-sm">
              {([
                ...
                ['Model / Make', part.make === 'All Models' ? 'Compatible with all models' : part.make],
                ['Application',  part.application.join(', ') || '—'],
                ['Stock',        part.inStock ? 'In Stock' : 'Out of Stock'],
                ['Images',       `${images.length} image${images.length !== 1 ? 's' : ''}`],
              ] as [string, string][]).map(([label, value]) => (
                <div key={label} className="flex gap-2 border-b border-gray-100 pb-2">
                  <dt className="font-bold text-gray-500 w-32 flex-shrink-0">{label}</dt>
          ...
          {showTab === 'description' && (
            <p className="text-gray-700 leading-relaxed whitespace-pre-wrap max-w-3xl">
